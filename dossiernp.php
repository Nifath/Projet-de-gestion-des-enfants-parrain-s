...
if(isset($_POST['submit'])) {
    ...
    $sql = "INSERT INTO dossier1 (nom, prenom, sexe, dateNaissance, age, lieuNaissance, nomPere, nomMere, nomTuteur, classe, etablissement, renseignement, photoProfil) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if ($stmt->execute()) {
        ...
        if (isset($_FILES["scannedDocuments"]) && is_array($_FILES["scannedDocuments"]["name"])) {
            for ($i = 0; $i < count($_FILES["scannedDocuments"]["name"]); $i++) {
                ...
                $sqlImages = "INSERT INTO images1 (id_dossier1, nomImage, tailleImage, typeImage, imageData) VALUES (?, ?, ?, ?, ?)";
    ...
    header("Location: generatepdfnonpar.php");
    exit;
